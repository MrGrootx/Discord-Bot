...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addUserOption(option => option.setName('target').setDescription('The member you \'like to kick')
        .setRequired(true)
        )
    ...
    async execute(interaction) {
        ...
        if(!interaction.member.permissions.has(PermissionsBitField.Flags.KickMembers)) return await interaction.reply({ content: `You don't have permission to kick members`, ephemeral: true})
    ...
    if (member, member.id == interaction.guild.ownerId) {
        return await interaction.reply({ content: "You can't kick the server owner!", ephemeral: true});
    }
    // ban higher command
    if (member) {
        if (member.roles.highest.position >= interaction.member.roles.highest.position) {
            return await interaction.reply({ content:`You cannot ban because they have a higher/same role!`,
            ephemeral: true});
        }
    }
